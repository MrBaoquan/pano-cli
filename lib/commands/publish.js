...
function createZipArchive(publishDir, versionTag) {
  // 使用 tar.gz 格式：tar 标准以 UTF-8 存储文件名，避免 zip 格式中文文件名在 Linux 解压乱码
  const archivePath = join(dirname(publishDir), `${basename(publishDir)}-v${versionTag}.tar.gz`);
  if (existsSync(archivePath)) rmSync(archivePath, { force: true });

  // Windows 10+ 自带 bsdtar，Linux/macOS 自带 GNU tar，均支持 -z gzip
  const tarRes = spawnSync('tar', ['-czf', archivePath, '-C', publishDir, '.'], { stdio: 'pipe', encoding: 'utf-8' });
  if (tarRes.status === 0 && existsSync(archivePath)) return archivePath;

  // 降级：Python tarfile（跨平台，显式 UTF-8）
  const pyCode = [
    ...
    '            tf.addfile(ti, open(full, "rb"))',
  ].join('\n');
  for (const exe of ['python3', 'python']) {
    const res = spawnSync(exe, ['-c', pyCode, publishDir, archivePath], { stdio: 'pipe', encoding: 'utf-8' });
    if (res.status === 0 && existsSync(archivePath)) return archivePath;
  }
  throw new BuildError('压缩包生成失败：未找到可用的 tar 或 Python 环境');
}
